  ...
  .factory('dataSetsServices',function($http,$q,$localStorage,$indexedDB){
    ...
    var dataSetsServices = {
      ...
      getIndividualDataSetFromServer : function(dataSetId,baseUrl){
        var defer = $q.defer();
        var field = "fields=id,created,name,timelyDays,formType,version,periodType,openFuturePeriods,expiryDays,dataEntryForm,dataElements[id,name,displayName,created,valueType,lastUpdated,optionSet[name,options[name,is]],categoryCombo[id,name,categoryOptionCombos[id,name]]],organisationUnits[id,name],sections[id,name],indicators[:all],categoryCombo[id,name,displayName,categoryOptionCombos[id,name]]"
